...
boundary conditions, pure numerical methods or more precisely, finite
element methods, have during recent years, due to evalution of both
the methods and the computors, become the natural choice for solving
Helmholtz equation.
...
can be done with the classical semi-analytical Fourier methods, even
...
\item Determination of numericially stable differential equations for
...
Additional well-known tools such as mode matching, are
necessary to solve general waveguide problems, but are not used in
this article.

In an example taken from acoustics, i.e., the field in the waveguide
is scalar, we show how these techniques kan be combined, in order to
...
mathematical base for a Fourier solution is outlined and in
...
% \section{The Building Block Method}
% \label{sec:BBM}
...
wavenumber, $\beta\in\mathbb{C}$, $\Re\beta\geq0$ is the (normalized)
...
$c_{0},$ the acoustic medium in the waveguide is characterized by the
density $\rho_{0}.$ Both $c_{0}$ and $\rho_{0}$ are constant.
...
\begin{figure}[htb]
  ...
  \label{fig:wg1}
\end{figure}
Using the notation $\mathbb{R}_{u_0}^{\pm}=\left\{
  u\in\mathbb{R}:\ u\gtrless u_{0}\right\} ,$ the straight
waveguide parts are $V_{\text{L}%
...
surface admittance $\beta=0$ on $\partial V_{\text{L}}\cap$ $V$ and on
$\partial V_{\text{R}}\cap$ $V$, whereas $\beta$ can be non-vanishing
and varying on $V_{\text{i}}.$
...
If $p_{+}\neq0,$ then we say that there is a source at $u=-\infty,$
...
and transmission oprerators for the waveguide, given that these
...
where an electro-magemtic scattering problem is treated, or
\cite{Nilsson:2002}, where similar problems
from acoustics are solved, we expand $\Phi(u,v)$ in cosine Fourier
series over $v$, assuming that
\begin{equation}
  \label{eq:fcosseries}
...
where the infinite matrices $A$ and $B^2$ have elements
\begin{equation}
  \label{eq:A}
...
Note that $(\varphi_m)$, with $\varphi_m(v)=\cos v\lambda_m(u)$, is in general not an orthogonal system since the eigenvalue problem (\ref{eq:bvp2}) is a regular Sturm-Liouville problem only if $Y$ is purely imaginary. However, $(\varphi_m)$ and $(\overline{\varphi_m})$, with a bar $\overline{\cdot}$ denoting the complex conjugate, form a biorthogonal system meaning that the bilinear form
\begin{equation}\label{eq:bilinear}
  \langle \varphi_m,\varphi_n\rangle =(\varphi_m,\overline{\varphi_n})=\int_0^1\varphi_m(v)\varphi_n(v){\rm d}v,
\end{equation}
vanishes if $n\neq m$. Here, for complex functions $f$ and $g$, $(f,g)$ denotes the complex scalar product and the bilinear form $\langle f,g \rangle$ is formally the same as the real scalar product. This means that the expansion coefficients like $\alpha_{mn}(u),\beta_{mn}(u)$ and $\mu_{mn}(u)$ in (\ref{eq:vsinlambda}--\ref{eq:mucoslambda}), are found from
...
Furthermore, the system $(\varphi_m)$ is proved to be complete in $L^2(0,1)$  using Hilbert space techniques \cite{Bauer:1964},\cite{Bauer:1965} but Paley-Wiener space techniques \cite{Young:1981} could also be used; see also \cite{Locker:2000} for a general treatise on eigenvalue problems based on functional analysis for differential equations with two-point boundary conditions such that like (\ref{eq:bvp2}), the related operator is non-self-adjoint. It is also worth noting \cite{Glav:2000} that the completeness property for $(\varphi_m)$ can be analysed as an analytic continuation of (\ref{eq:am}) from purely imaginary $Y$, for which completeness holds, to general $Y$.
...
and third derivatives if the boundary has non-zero amittance.
...
Recall that the block is assumed to be an infintely long waveguide
...
definition: Let for all $u\in\R$, the wavefield
\begin{equation}
  \label{eq:Phipart1}
...
\begin{align}
  ...
  \intertext{follow. For $R^-$ and $T^-$, we proceed similarily
...
numerically from right and left respectively, using the intial values
...
knowledge about existance or placements of these is at hand, see for
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{waveguide.4}
...
cross-section, see figure~\ref{fig:wg4}. For simplicity, we assume
that $\Omega_2$ is parallel to the $x$-axis. Furthermore, assume that
reflection and transmission operators in matrix form for $\Omega_1$
and $\Omega_2$ are known. We use the following denotation:
\begin{center}
  \begin{tabular}{ll}
    Symbol&Explanation\\
    \hline
    ...
    from the left.\\
    $R^{+}_{\text{tot}}$&Reflection operator for
    $\cup_{j=1}^3\Omega_j$ for waves entering
    from the left.\\
    $T^{+}_{\text{tot}}$&Transmission operator for $\cup_{j=1}^3\Omega_j$ for
    waves entering from the left.\\
    \hline
  \end{tabular}
\end{center}

For the straight part $\Omega_2$ having length $\ell$ and width $a$,
we define the operator
\begin{equation}
  \label{eq:S}
  S(x)=
  \begin{pmatrix}
    \e^{\i\alpha_0x}&0&0&\cdots\\
    0&\e^{\i\alpha_1x}&0&\cdots\\
    0&0&\e^{\i\alpha_2x}&\\
    \vdots&\vdots&&\ddots
  \end{pmatrix},\quad 0\le x\le\ell,
\end{equation}
where $\alpha_n=\sqrt{k^2-\dfrac{n^2\pi^2}{a^2}}$ and $x$ is the
horizontal distance from the border between $\Omega_1$ and $\Omega_2$.

Assume that a right-marching field $\Phiin$ is entering $\Omega_1$
from the left, and that there are no sources to the right of
$\Omega_3$. Define operators $C^\pm$ such that at the border between
$\Omega_1$ and $\Omega_2$ the right-marching and left-marching waves
are $C^+\Phiin$ and $C^-\Phiin$ respectively. From standard theory for
straight waveguides follows that in $\Omega_2$ at position $x$, the
field is
\begin{equation}
  \label{eq:midfield}
  (S(x)C^++S^{-1}(x)C^-)\Phiin.
...
\begin{equation}
  ...
  \begin{split}
    ...
    &C^-=S(\ell)R_3^{+}S(\ell)C^+,\\
    &T^+_{\text{tot}}=T_3^{+}S(\ell)C^+,\\
    &R^+_{\text{tot}}=R_1^{+}+T_1^{-}C^-.
  \end{split}
\end{equation}
...
In this geometry, we have run two test cases, (a) with hard walls,
i.e., with zero admittance on all boundaries, and (b) with a varying
admittance condition on the upper boundary. In the latter case, there
is a smoothly and monotonuosly increasing admittance in the intervals
$F_j([-2,-1]+\i)$, a constant admittance $\beta=0.5+0.5i$ in the
intervals $F_j([-1,1]+\i)$, and finally smoothly monotonously
decreasing admittance in the intervals $F_j([1,2]+\i)$, where the
functions $F_j, j=1,2$ are the conformal mappings defined in
Section~\ref{sec:conformal-mappings}. The remaining boundaries are
hard.
...
% Section~\ref{sec:conformal-mappings}. Additionally, results for
% $\beta=0$ on all the boundary have also been calculated.

\subsection{Conformal mappings}
\label{sec:conformal-mappings}

The region is divided into three disjunct parts. $\Omega_1$ contains
...
\begin{figure}[htb]
  ...
  \caption{The two building blocks. The gridlines are images under the
...
determined for $u=-5,-4.9,...,5$ in $\Omega_1$, and for
$u=-7,-6.9,...,7$ in $\Omega_3$. Linear interpolation was then used
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.49\linewidth]{Tkadm0}
  \includegraphics[width=0.49\linewidth]{Tkadm05plus05i}
  \caption{$\abs{T(1,1)}$ for $\cup_{j=1}^3\Omega_j$ calculated for
    $k=0..10$ with hard walls (left) and for $k=0..5$ with admittance
    $0.5+0.5i$ (right).}
  \label{fig:Tk}
\end{figure}

We start this presentation with the results shown in
Figure~\ref{fig:Tk}, where the absolute value of the first element in
the transmission operator for the entire waveguide is calculated for a
range of frequencies ($k$ values), both with the Fourier methods
presented in this article and with the finite element method. For hard
boundaries, the results agree well for $k$ values up to at least $15$,
but in the case with a non-zero admittance, the two methods disagree
already for $k>1$. The sources of this discrepancy will be discussed
further in the next section, but we will here present some results
from the $k$ intervals in which the methods agree mainly.
...
\begin{figure}[htb]
  ...
  \includegraphics[keepaspectratio=false,width=\linewidth,
  height=3cm]{phi3}
  \caption{$\Re\Phi_1,\Re\Phi_2,\Re\Phi_3,\dots$ for $k=12$ and hard
    walls (above) and $k=1$ and admittance as described in the
    article. Dotted vertical lines indicate borders between
    $\Omega_1$, $\Omega_2$ and $\Omega_3$.}
  \label{fig:phi}
\end{figure}

In Figure~\ref{fig:phi}, $\Re{\Phi_1},\Re{\Phi_2},\dots$ is plotted
for $k=5$, $k=10$ and $k=15$ in the hard wall case. The measure on the
horizontal axis in the plot is the distance along the central curve in
the waveguide. In Figure~\ref{fig:phitot3h}, a contour plot of $\Re
p(x,y)$ is shown for $k=15$ and hard walls, and in
Figure~\ref{fig:phitot1a}, i similar plot is drawn for $k=1$ and
admittance conditions as described in
Section~\ref{sec:boundary-conditions}.

\begin{figure}[htb]
  \centering
  \includegraphics[width=\linewidth]{phitot3}
  \caption{$\Re p(x,y)$ plotted for $k=15$ and hard walls.}
  \label{fig:phitot3h}
\end{figure}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=\linewidth]{phitot3}
  \caption{$\Re p(x,y)$ plotted for $k=1$ and admittance conditions as
    described in Section~\ref{sec:boundary-conditions}.}
  \label{fig:phitot1a}
\end{figure}
...
the left end of $\Omega_2$, i.e. with $(A+B)\Phiin$. This comparison
is done in Table~\ref{tab:phires-scend}, and shows that the RT method
and the DtN method confirm each other well. The difference between the
two results gives a hint of the accuracy of the calculations.

\begin{table}[ht]
  \begin{equation*}
    \begin{array}{|c|rrc|}
      \hline
      k=5&\multicolumn1c{\vec\Phi_{\Omega_1}(\text{end})}
      &\multicolumn1c{\vec\Phi_{\Omega_2}(0)}
      &\abs{\text{difference}}\\
      \hline
      \Phi_1&0.0113 + 0.4217\i&0.0115 + 0.4218\i&2\cdot10^{-4}\\
      \Phi_2&-0.0048 + 0.0077\i&-0.0048 + 0.0077\i&2\cdot10^{-5}\\
      \hline
      k=10&\multicolumn1c{\vec\Phi_{\Omega_1}(\text{end})}
      &\multicolumn1c{\vec\Phi_{\Omega_2}(0)}
      &\abs{\text{difference}}\\
      \hline
      \Phi_1&-0.0654 - 0.3796\i&  -0.0656 - 0.3794\i&3\cdot10^{-4}\\
      \Phi_2& 0.4863 - 0.0468\i&   0.4867 - 0.0472\i&6\cdot10^{-4}\\
      \Phi_3& 0.0001 - 0.0003\i&   0.0001 - 0.0003\i&2\cdot10^{-6}\\
      \hline
      k=15&\multicolumn1c{\vec\Phi_{\Omega_1}(\text{end})}
      &\multicolumn1c{\vec\Phi_{\Omega_2}(0)}
      &\abs{\text{difference}}\\
      \hline
      \Phi_1&0.0761 + 0.1371\i&   0.0763 + 0.1368\i&3\cdot10^{-4}\\
      \Phi_2&-0.2413 - 0.5671\i&  -0.2417 - 0.5670\i&4\cdot10^{-4}\\
      \Phi_3&-0.2527 + 0.3930\i&  -0.2525 + 0.3937\i&7\cdot10^{-4}\\
      \hline
      k=20&\multicolumn1c{\vec\Phi_{\Omega_1}(\text{end})}
      &\multicolumn1c{\vec\Phi_{\Omega_2}(0)}
      &\abs{\text{difference}}\\
      \hline
      \Phi_1& 0.0572+0.1007\i&0.0574 + 0.1008\i&2\cdot10^{-4}\\
      \Phi_2& 0.1233+0.4054\i&0.1240 + 0.4049\i&8\cdot10^{-4}\\
      \Phi_3& 0.1733+0.6416\i&0.1743 + 0.6419\i&1.1\cdot10^{-3}\\
      \Phi_4&-0.3960+0.2255\i&-0.3959+0.2266\i&1.1\cdot10^{-3}\\
      \hline
    \end{array}
  \end{equation*}
  \caption{$\Phi_1$, $\Phi_2$, $\Phi_3$ and $\Phi_4$ at
    the border between $\Omega_1$ and $\Omega_2$
    calculated with two different methods for
    $k=5,10,15,20$.}
  \label{tab:phires-scend}
\end{table}

A similar comparison is given in Table~\ref{tab:phires-bcend} where
$\vec\Phi$ in the end of $\Omega_3$ is compared to
$T^{\text{tot}}\Phiin$ and with corresponding results from the FEM
calculus.
...
\begin{table}[h]
  \begin{equation*}
    \begin{array}{|c|rrc|}
      \hline
      k=5&\multicolumn1c{\vec\Phi_{\Omega_3}(\text{end})}
      &\multicolumn1c{T^{\text{tot}}\Phiin}
      &\abs{\text{difference}}\\
      \hline
      \Phi_1& -0.0744 + 0.3166\i&-0.0744 + 0.3166\i&4\cdot10^{-5}\\
      \hline
      k=10&\multicolumn3c{}\vline\\
      % &\multicolumn1c{\vec\Phi_{\Omega_3}(\text{end})}
      % &\multicolumn1c{T^{\text{tot}}\Phiin}
      % &\abs{\text{difference}}\\
      \hline
      \Phi_1& 0.0902 + 0.0420\i&0.0903 + 0.0421\i&1\cdot10^{-4}\\
      \hline
      k=15&\multicolumn3c{}\vline\\
      % &\multicolumn1c{\vec\Phi_{\Omega_3}(\text{end})}
      % &\multicolumn1c{T^{\text{tot}}\Phiin}
      % &\abs{\text{difference}}\\
      \hline
      \Phi_1&0.0066 + 0.1879\i&0.0067 + 0.1879\i&1\cdot10^{-4}\\
      \Phi_2& 0.0626 + 0.0580\i&0.0626 + 0.0580\i&1\cdot10^{-4}\\
      \hline
      k=20&\multicolumn3c{}\vline\\
      % &\multicolumn1c{\vec\Phi_{\Omega_3}(\text{end})}
      % &\multicolumn1c{T^{\text{tot}}\Phiin}
      % &\abs{\text{difference}}\\
      \hline
      \Phi_1&0.1107 - 0.0280\i&0.1105 - 0.0281\i&2\cdot10^{-4}\\
      \Phi_2&-0.4216-0.0965\i&-0.4222-0.0957\i&1.0\cdot10^{-3}\\
      \hline
    \end{array}
  \end{equation*}
  \caption{$\Phi_1$ and $\Phi_2$ at the end of $\Omega_3$
    calculated with two different methods for
    $k=5,10,15,20$.}
...
%     \begin{array}{|c|rrc|}
%       \hline
%       k=5&\multicolumn1c{\vec\Phi_{\Omega_1}(\text{start})}
%       &\multicolumn1c{(R^{\text{tot}}+I)\Phiin}
%       &\abs{\text{difference}}\\
%       \hline
%       \Phi_1&1.1324-0.0638\i&1.1324-0.0638\i&1.8\cdot10^{-6}\\
%       \Phi_2&(0.1+3.2\i)10^{-7}&(0.1+3.2\i)10^{-7}&4.3\cdot10^{-12}\\
%       \Phi_3&(-1.9-0.3\i)10^{-8}&(-1.9-0.3\i)10^{-8}&2.4\cdot10^{-13}\\
%       \Phi_4&<10^{-14}&<10^{-14}&\\
%       \hline
%       k=10&\multicolumn1c{\vec\Phi_{\Omega_1}(\text{start})}
%       &\multicolumn1c{(R^{\text{tot}}+I)\Phiin}
%       &\abs{\text{difference}}\\
%       \hline
%       \Phi_1&1.1252-0.0997\i&1.1240-0.1002\i&1.2\cdot10^{-3}\\
%       \Phi_2&(-1.3+1.1\i)10^{-5}&(-1.3+1.2\i)10^{-5}&2.5\cdot10^{-7}\\
%       \Phi_3&(-5.1-2.7\i)10^{-8}&(-5.1-2.7\i)10^{-8}&2.1\cdot10^{-10}\\
%       \Phi_4&<10^{-12}&<10^{-12}&\\
%       \hline
%       k=15&\multicolumn1c{\vec\Phi_{\Omega_1}(\text{start})}
%       &\multicolumn1c{(R^{\text{tot}}+I)\Phiin}
%       &\abs{\text{difference}}\\
%       \hline
%       \Phi_1&0.8874+0.1162\i&0.8870+0.1126\i&3.6\cdot10^{-3}\\
%       \Phi_2&0.0202-0.0013\i&0.0202-0.0017\i&3.9\cdot10^{-4}\\
%       \Phi_3&(0.0-1.8\i)10^{-7}&(0.0-1.8\i)10^{-7}&1.2\cdot10^{-9}\\
%       \Phi_4&<10^{-8}&<10^{-8}&\\
%       \hline
%       k=20&\multicolumn1c{\vec\Phi_{\Omega_1}(\text{start})}
%       &\multicolumn1c{(R^{\text{tot}}+I)\Phiin}
%       &\abs{\text{difference}}\\
%       \hline
%       \Phi_1&1.0638-0.2641\i&1.0646-0.2645\i&9.7\cdot10^{-4}\\
%       \Phi_2&0.5598+0.1421\i&0.5586+0.1448\i&2.9\cdot10^{-3}\\
%       \Phi_3&(9.9-9.7\i)10^{-8}&(9.9-9.7\i)10^{-8}&4.6\cdot10^{-10}\\
%       \Phi_4&(-1.1-0.1\i)10^{-7}&(-1.1-0.1\i)10^{-7}&5.4\cdot10^{-10}\\
%       \hline
%     \end{array}
%   \end{equation*}
%   \caption{$\Phi_1$, $\Phi_2$, $\Phi_3$ and $\Phi_4$ at
%     the left end of $\Omega_1$
%     calculated with two different methods for
%     $k=5,10,15,20$.}
%   \label{tab:phires-scstart}
% \end{table}


% An indication of the accuracy of the calculations as well as a
% comparison between the RT and DtN methods are given in
% Table~\ref{tab:phires-scend}.  There, $\Phi_1,\dots\Phi_4$,
% calculated with the two different methods, are given at the joint
% between $\Omega_1$ and $\Omega_2$ for $k=1,\dots,4$. We note that
% for low frequencies, the two methods support each other quite well,
% but that the accuracy deteriorates with increasing $k$, if not the
% size of the truncated matrices is increased accordingly.
...
% treating problems like the one in the example, and it is probably
% the most used method to solve such problems. As a comparison, we
% have applied a FEM solver to the problem. Table.. shows
...
boundary variations not commented so far. Below follows a list of some
...
However, as is seen in Figure..., in the $k$ intervals where the
Fourier method and the finite element method agree, the size of the
truncated matrices seems not to be the critical. In the figure,
$\abs{T^{\text{tot}}(1,1)}$ is plotted over increasing matrix sizes
from $2\times2$ to $30\times30$. The results seem to be stable to
three significant figures already for $5\times5$ matrices.

% However, as is shown in ..., where the problem is studied for $k=5$
% using matrices truncated to sizes
% $2\times2,4\times4,\dots,30\times30$, the results correspond to
% three significant digits already for $4\times4$ matrices.

As a reference and comparision, the problem in the example has been
solved using commercial software for the finite element method
(FEM). It turns out, see Figure~\ref{fig:Tk}, that with hard
boundaries, the correspondence is good for a relatively large
frequency domain, but that for non-hard boundaries, the two methods
correspond only for low frequencies.

It is beyond the scope of this paper to fully investigate the sources
of the disparities revealed in Figure~\ref{fig:Tk}. It is a well-known
fact that the number of mesh elements required to get an accurate FEM
solution to a wave scattering problem increases rapidly with growing
fequencies, see for example \cite{Ihlenburg:1998} or
